...
running ones, click to launch or focus — which also dismisses the dock.
Auto-hides; mouse to the far-left edge to bring it back for as long as the
pointer stays there, or **hold Ctrl (~0.3s) to pin it open** — it then stays
until a **clean tap of Ctrl** (Ctrl down/up with no other key) closes it again,
or you click an app; Ctrl chords (Ctrl+C, Ctrl+arrow, …) are ignored.
